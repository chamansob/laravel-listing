                ...
                <div class="statbox widget box box-shadow">
                    <div class="widget-content widget-content-area">
                        <table id="html5-extension" class="table dt-table-hover">
                            ...
                            <tbody>
                                @foreach ($can_provides as $provide)
                                    <tr class="social-{{ $provide->id }}">
                                        ...
                                        <td class="text-center">
                                            <button type="button" onClick="statusFunction({{ $provide->id }},'CanProvide')"
                                        ...
                                        <td class="text-center">
                                            <div class="action-btns">
                                                ...
                                                <a href="{{ route('can_provides.edit', $provide->id) }}"
                                                    class="action-btn btn-edit bs-tooltip me-2" data-toggle="tooltip"
                                                    data-placement="top" title="Edit" data-bs-original-title="Edit">
                                                    <i data-feather="edit"></i>
                                                </a>

                                                <a href="#" onClick="deleteFunction({{ $provide->id }},'CanProvide')"
                        ...
                        @if ($can_provides->count() != 0)
                            <div class="ms-3">
                                <button id="deleteall" onClick="deleteAllFunction('CanProvide')" class="btn btn-danger mb-2 me-4">
        ...
        <script type="text/javascript">
            function deleteAllFunction(table) {
                ...
                if (checkedValues.length === 0) {
                    ...
                    var crf = '{{ csrf_token() }}';
                    $.post("{{ route('can_provides.delete') }}", {
                        _token: crf,
                ...
                document.querySelector('.warning.changestatus' + id).addEventListener('click', function() {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            setTimeout(function() {
                                var crf = '{{ csrf_token() }}';
                                $.post("{{ route('can_provides.status') }}", {
                                    _token: crf,
                                    id: id,table:table,
                                     table: table
                                }, function(data) {
